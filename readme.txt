...
This project is a high-performance implementation of Dijkstra's algorithm built for large-scale geographic road networks. By combining early termination, incremental state reset, and an A* heuristic that exploits Euclidean geometry, it handles thousands of repeated shortest-path queries significantly faster than a standard implementation.
...
2. Incremental reset — tracks only vertices modified per query and resets only those, avoiding a full O(V) reinitialization between every query.
3. A* heuristic — uses Euclidean distance to the destination to bias the search toward the target and shrink the explored region.
